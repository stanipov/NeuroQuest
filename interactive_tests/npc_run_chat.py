#!/usr/bin/env python3
"""
Interactive NPCAgent run() test session.

Loads a lore file, instantiates an LLM client and NPCAgent,
then runs a looped session calling npc_agent.run(),
...
def create_unique_log_file(tmp_dir: Path) -> Path:
    ...
    filename = f"npc_run_{timestamp}.json"
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"NPCAgent Run Interactive Session ({npc_name})")
    print("=" * 60)
    print(f"\nTesting run() for {npc_name}.")
    print("Enter player actions/inputs to test NPC responses.")
    ...
    while True:
        try:
            ...
            # Run full NPC agent (gateway + response)
            print(f"\n{npc_name} is thinking...")

            t_s = time.time()
            response_raw = npc_agent.run(user_input)
            t_e = time.time()

            response = response_raw.get("message", {})
            if response.get("action"):
                print(f"\n--- {npc_name}'s Response ---")
                print(f"  Action: {response['action']}")
                if response.get('mental_state'):
                    print(f"  Mental State: {response['mental_state']}")
                if response.get('reason'):
                    print(f"  Reason: {response['reason']}")
                print(f"Response processed in {t_e - t_s:.2f} s")
                print("---\n")
            else:
                print(f"\n{npc_name} chose not to respond to this input.\n")
